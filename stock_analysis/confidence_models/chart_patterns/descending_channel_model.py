...
MODEL_PATH = os.path.join(MODEL_DIR, "descending_channel_model.pkl")
...
def descending_channel_score(
    upper_neg_pct_slope: float,
    lower_neg_pct_slope: float,
    ...
    """
    Strict rule-based quality score for a textbook-style Descending Channel.
    Output: [0, 100]
    """

    avg_drop = (upper_neg_pct_slope + lower_neg_pct_slope) / 2

    # 1) Both lines should clearly fall
    if avg_drop >= 0.0010:
        score_drop = 100
...
def _sample(region: str):
    if region == "ideal":
        upper_neg_pct_slope = np.random.uniform(0.0008, 0.0020)
        lower_neg_pct_slope = np.random.uniform(0.0008, 0.0020)
        ...
        upper_neg_pct_slope = np.random.uniform(0.0004, 0.0012)
        lower_neg_pct_slope = np.random.uniform(0.0004, 0.0012)
        ...
        upper_neg_pct_slope = np.random.uniform(0.00015, 0.0006)
        lower_neg_pct_slope = np.random.uniform(0.00015, 0.0006)
        ...
        upper_neg_pct_slope = np.random.uniform(0.00000, 0.00018)
        lower_neg_pct_slope = np.random.uniform(0.00000, 0.00018)
        ...
        upper_neg_pct_slope = np.random.uniform(0.0003, 0.0014)
        lower_neg_pct_slope = np.random.uniform(0.0003, 0.0014)
        ...
        upper_neg_pct_slope = np.random.uniform(0.0000, 0.0030)
        lower_neg_pct_slope = np.random.uniform(0.0000, 0.0030)
        ...
        upper_neg_pct_slope = np.random.uniform(0.0000, 0.00005)
        lower_neg_pct_slope = np.random.uniform(0.0000, 0.00005)
    ...
    return (
        float(upper_neg_pct_slope),
        float(lower_neg_pct_slope),
...
def build_dataset() -> pd.DataFrame:
    ...
    for region, n in REGION_COUNTS.items():
        for _ in range(n):
            ...
            base_score = descending_channel_score(
                upper_neg_pct_slope=upper_neg_pct_slope,
                lower_neg_pct_slope=lower_neg_pct_slope,
...
def load_model():
    if not os.path.exists(MODEL_PATH):
        raise FileNotFoundError(
            "Descending Channel regression model not found. Run "
            "`python -m stock_analysis.confidence_models.chart_patterns.descending_channel_model` "
...
if __name__ == "__main__":
    ...
    test_cases = [
        ...
        ("Weak drop",        0.00008, 0.00006, 0.85, 0.85, 0.85, 0.85),
    ...
    for desc, upper_neg_pct_slope, lower_neg_pct_slope, slope_parallel_score, touch_balance_ratio, width_stability_score, touch_richness_score in test_cases:
        rule_score = descending_channel_score(
            upper_neg_pct_slope=upper_neg_pct_slope,
            lower_neg_pct_slope=lower_neg_pct_slope,
            slope_parallel_score=slope_parallel_score,
            touch_balance_ratio=touch_balance_ratio,
