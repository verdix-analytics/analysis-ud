...
MODEL_PATH = os.path.join(MODEL_DIR, "inverse_head_and_shoulders_model.pkl")
...
def inverse_head_and_shoulders_score(
    shoulder_vs_head_min: float,
    ...
    """
    Strict rule-based quality score for a textbook-style
    Inverse Head and Shoulders.
    Output: [0, 100]
    """

    # 1) Head depth: most important
    if shoulder_vs_head_min >= 0.040:
        score_head = 100
    elif shoulder_vs_head_min >= 0.030:
        score_head = 80
    elif shoulder_vs_head_min >= 0.020:
        score_head = 55
    elif shoulder_vs_head_min >= 0.010:
        score_head = 25
    else:
        score_head = 0
...
def _sample(region: str):
    if region == "ideal":
        shoulder_vs_head_min = np.random.uniform(0.035, 0.070)
        ...
        shoulder_vs_head_min = np.random.uniform(0.025, 0.045)
        ...
        shoulder_vs_head_min = np.random.uniform(0.015, 0.030)
        ...
        shoulder_vs_head_min = np.random.uniform(0.003, 0.015)
        ...
        shoulder_vs_head_min = np.random.uniform(0.020, 0.050)
        ...
        shoulder_vs_head_min = np.random.uniform(0.000, 0.080)
        ...
        shoulder_vs_head_min = np.random.uniform(0.000, 0.008)
...
def build_dataset() -> pd.DataFrame:
    ...
    for region, n in REGION_COUNTS.items():
        for _ in range(n):
            ...
            base_score = inverse_head_and_shoulders_score(
...
def load_model():
    if not os.path.exists(MODEL_PATH):
        raise FileNotFoundError(
            "Inverse Head and Shoulders regression model not found. Run "
            "`python -m stock_analysis.confidence_models.chart_patterns.inverse_head_and_shoulders_model` "
...
if __name__ == "__main__":
    ...
    for desc, shoulder_vs_head_min, shoulder_diff_pct, neckline_diff_pct, left_span, right_span, symmetry_ratio in test_cases:
