...
MODEL_PATH = os.path.join(MODEL_DIR, "diamond_top_model.pkl")
...
def diamond_top_score(
    ...
    """
    Strict rule-based quality score for a textbook-style Diamond Top.
    Output: [0, 100]
    """

    # 1) Center peak should be near the middle
    ...
    # 5) Right-side boundary tightness
...
def load_model():
    if not os.path.exists(MODEL_PATH):
        raise FileNotFoundError(
            "Diamond Top regression model not found. Run "
            "`python -m stock_analysis.confidence_models.chart_patterns.diamond_top_model` "
