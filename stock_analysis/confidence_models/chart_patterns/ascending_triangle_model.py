...
MODEL_PATH = os.path.join(MODEL_DIR, "ascending_triangle_model.pkl")
...
def ascending_triangle_score(
    high_abs_pct_slope: float,
    low_pct_slope: float,
    ...
    """
    Strict rule-based quality score for a textbook-style Ascending Triangle.
    Output: [0, 100]
    """

    # 1) Upper line should be flat
    if high_abs_pct_slope <= 0.0004:
        score_high_flat = 100
    elif high_abs_pct_slope <= 0.0008:
        score_high_flat = 80
    elif high_abs_pct_slope <= 0.0015:
        score_high_flat = 55
    elif high_abs_pct_slope <= 0.0025:
        score_high_flat = 25
    else:
        score_high_flat = 0
    ...
    # 3) High consistency
    if high_deviation <= 0.0025:
        score_high_consistency = 100
    elif high_deviation <= 0.0045:
        score_high_consistency = 80
    elif high_deviation <= 0.0080:
        score_high_consistency = 55
    elif high_deviation <= 0.0120:
        score_high_consistency = 25
    else:
        score_high_consistency = 0
    ...
    overall = (
        0.28 * score_high_flat +
        0.28 * score_low_rise +
        0.18 * score_high_consistency +
        0.12 * score_gap +
        0.08 * score_pivots +
        0.06 * score_balance
    )
...
def _sample(region: str):
    if region == "ideal":
        high_abs_pct_slope = np.random.uniform(0.0000, 0.0004)
        low_pct_slope = np.random.uniform(0.0008, 0.0020)
        high_deviation = np.random.uniform(0.0005, 0.0025)
        ...
        high_abs_pct_slope = np.random.uniform(0.0004, 0.0010)
        low_pct_slope = np.random.uniform(0.0005, 0.0012)
        high_deviation = np.random.uniform(0.0020, 0.0045)
        ...
        high_abs_pct_slope = np.random.uniform(0.0010, 0.0025)
        low_pct_slope = np.random.uniform(0.00015, 0.0006)
        high_deviation = np.random.uniform(0.0040, 0.0090)
    ...
    elif region == "sloping_top":
        high_abs_pct_slope = np.random.uniform(0.0025, 0.0080)
        low_pct_slope = np.random.uniform(0.0004, 0.0012)
        high_deviation = np.random.uniform(0.0020, 0.0060)
        gap_pct_now = np.random.uniform(0.015, 0.040)
        pivot_count = np.random.randint(4, 8)
        n_highs = np.random.randint(2, 4)
        n_lows = np.random.randint(2, 4)

    elif region == "flat_or_weak_lows":
        high_abs_pct_slope = np.random.uniform(0.0000, 0.0012)
        low_pct_slope = np.random.uniform(-0.0004, 0.00015)
        high_deviation = np.random.uniform(0.0020, 0.0060)
        ...
        high_abs_pct_slope = np.random.uniform(0.0000, 0.0015)
        low_pct_slope = np.random.uniform(0.0003, 0.0012)
        high_deviation = np.random.uniform(0.010, 0.030)
        gap_pct_now = np.random.uniform(0.020, 0.050)
        pivot_count = np.random.randint(4, 8)
        n_highs = np.random.randint(2, 5)
        n_lows = np.random.randint(2, 5)
        ...
        high_abs_pct_slope = np.random.uniform(0.0000, 0.0100)
        low_pct_slope = np.random.uniform(-0.0020, 0.0030)
        high_deviation = np.random.uniform(0.0005, 0.0500)
        ...
        high_abs_pct_slope = np.random.uniform(0.0030, 0.0150)
        low_pct_slope = np.random.uniform(-0.0030, 0.0000)
        high_deviation = np.random.uniform(0.020, 0.080)
    ...
    return (
        float(high_abs_pct_slope),
        float(low_pct_slope),
        float(high_deviation),
        float(gap_pct_now),
        float(pivot_count),
...
def build_dataset() -> pd.DataFrame:
    ...
    for region, n in REGION_COUNTS.items():
        for _ in range(n):
            ...
            base_score = ascending_triangle_score(
...
def load_model():
    if not os.path.exists(MODEL_PATH):
        raise FileNotFoundError(
            "Ascending Triangle regression model not found. Run "
            "`python -m stock_analysis.confidence_models.chart_patterns.ascending_triangle_model` "
...
if __name__ == "__main__":
    ...
    test_cases = [
        ("Ideal",             0.0002, 0.0012, 0.0015, 0.010, 8, 1.00),
        ("Good",              0.0007, 0.0007, 0.0035, 0.020, 6, 0.75),
        ("Borderline",        0.0018, 0.0003, 0.0070, 0.035, 5, 0.67),
        ("Sloping top",       0.0040, 0.0008, 0.0030, 0.020, 6, 1.00),
        ("Weak lows",         0.0007, 0.00005, 0.0035, 0.020, 6, 1.00),
        ("Messy highs",       0.0007, 0.0008, 0.0200, 0.020, 6, 1.00),
        ("Wide gap",          0.0007, 0.0008, 0.0035, 0.090, 6, 1.00),
        ("Imbalanced touch",  0.0007, 0.0008, 0.0035, 0.020, 7, 0.33),
        ("Compressed",        0.0007, 0.0008, 0.0035, 0.008, 4, 1.00),
        ("User example",      0.0013, 0.00042, 0.0058, 0.028, 6, 0.60),
    ]

    print(f"\n{'Pattern':<18} {'RuleScore':>10} {'Predicted':>10} {'Delta':>10}")
    print("-" * 56)

    for desc, high_abs_pct_slope, low_pct_slope, high_deviation, gap_pct_now, pivot_count, touch_balance_ratio in test_cases:
