...
MODEL_PATH = os.path.join(MODEL_DIR, "falling_wedge_model.pkl")
...
def falling_wedge_score(
    high_neg_pct_slope: float,
    low_neg_pct_slope: float,
    ...
    """
    Strict rule-based quality score for a textbook-style Falling Wedge.
    Output: [0, 100]
    """

    avg_drop = (high_neg_pct_slope + low_neg_pct_slope) / 2

    # 1) Both lines falling
    if avg_drop >= 0.0010:
        score_drop = 100
    elif avg_drop >= 0.0006:
        score_drop = 80
    elif avg_drop >= 0.0003:
        score_drop = 55
    elif avg_drop >= 0.00015:
        score_drop = 25
    else:
        score_drop = 0

    # 2) Upper line should fall faster than lower line
...
def _sample(region: str):
    if region == "ideal":
        ...
        low_neg_pct_slope = np.random.uniform(0.0003, 0.0010)
        ...
        high_neg_pct_slope = np.random.uniform(0.0005, 0.0015)
        low_neg_pct_slope = np.random.uniform(0.0002, 0.0008)
        ...
        high_neg_pct_slope = np.random.uniform(0.0002, 0.0010)
        low_neg_pct_slope = np.random.uniform(0.00015, 0.0006)
    ...
    elif region == "weak_drop":
        high_neg_pct_slope = np.random.uniform(0.00000, 0.00020)
        low_neg_pct_slope = np.random.uniform(0.00000, 0.00018)
        ...
        high_neg_pct_slope = np.random.uniform(0.0002, 0.0010)
        low_neg_pct_slope = np.random.uniform(0.0002, 0.0010)
        ...
        high_neg_pct_slope = np.random.uniform(0.0000, 0.0025)
        low_neg_pct_slope = np.random.uniform(0.0000, 0.0020)
        ...
        high_neg_pct_slope = np.random.uniform(0.0000, 0.00005)
        low_neg_pct_slope = np.random.uniform(0.0000, 0.00005)
...
def build_dataset() -> pd.DataFrame:
    ...
    for region, n in REGION_COUNTS.items():
        for _ in range(n):
            ...
            base_score = falling_wedge_score(
                high_neg_pct_slope=high_neg_pct_slope,
                low_neg_pct_slope=low_neg_pct_slope,
...
def load_model():
    if not os.path.exists(MODEL_PATH):
        raise FileNotFoundError(
            "Falling Wedge regression model not found. Run "
            "`python -m stock_analysis.confidence_models.chart_patterns.falling_wedge_model` "
...
if __name__ == "__main__":
    ...
    test_cases = [
        ("Ideal",            0.0012, 0.0005, 0.95, 0.95, 0.95, 0.95),
        ("Good",             0.0008, 0.00035, 0.82, 0.82, 0.80, 0.80),
        ("Borderline",       0.00035, 0.0002, 0.60, 0.60, 0.60, 0.60),
        ("Weak drop",        0.00010, 0.00008, 0.80, 0.80, 0.80, 0.80),
        ("Bad divergence",   0.00065, 0.0006, 0.20, 0.80, 0.80, 0.80),
        ("No convergence",   0.0010, 0.0004, 0.82, 0.20, 0.80, 0.80),
        ("Few touches",      0.0010, 0.0004, 0.82, 0.82, 0.20, 0.80),
        ("Bad balance",      0.0010, 0.0004, 0.82, 0.82, 0.80, 0.20),
        ("User example",     0.00071, 0.00032, 0.72, 0.70, 0.68, 0.66),
    ]

    print(f"\n{'Pattern':<16} {'RuleScore':>10} {'Predicted':>10} {'Delta':>10}")
    print("-" * 54)

    for desc, high_neg_pct_slope, low_neg_pct_slope, slope_divergence_score, convergence_score, touch_richness_score, touch_balance_ratio in test_cases:
