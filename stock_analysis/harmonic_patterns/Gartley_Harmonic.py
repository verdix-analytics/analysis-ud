...
from stock_analysis.confidence_models.harmonic_patterns.gartley_model import load_model, learned_confidence
...
class GartleyHarmonicPattern(BasePattern):
    enabled = True
    ...
    def is_gartley_candidate(self, retraces, tolerance: float = 0.08):
        for leg, (lo, hi) in self.GARTLEY_RANGES.items():
            val = retraces[leg]
    ...
    def _run(self, df: pd.DataFrame) -> dict:
        ...
        candidates = extract_recent_xabcd_candidates(
            pivots,
            max_candidates=100,
            filter_fn=self.is_gartley_candidate
        )
        ...
        for candidate in candidates:
            ...
            ideals = { "XAB": 0.618, "ABC": 0.618, "BCD": 1.618, "XABCD": 0.786 }
            
            rule_based_conf = harmonic_ratio_similarity(actual, ideals, self.GARTLEY_RANGES)
