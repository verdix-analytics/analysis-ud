...
from stock_analysis.confidence_models.harmonic_patterns.crab_model import load_model, learned_confidence
...
class CrabHarmonicPattern(BasePattern):
    enabled = True
    ...
    def is_crab_candidate(self, retraces, tolerance: float = 0.08):
        for leg, (lo, hi) in self.CRAB_RANGES.items():
            val = retraces[leg]
            if not (lo * (1 - tolerance) <= val <= hi * (1 + tolerance)):
                return False
        return True
    
    def crab_validate(self, X, A, B, C, D):
        bullish = X < A
    ...
    def _run(self, df: pd.DataFrame) -> dict:
        ...
        candidates = extract_recent_xabcd_candidates(
            pivots,
            max_candidates=100,
            filter_fn=self.is_crab_candidate,
            validate_fn=self.crab_validate
        )
        ...
        for candidate in candidates:
            ...
            ideals = { "XAB": 0.618, "ABC": 0.618, "BCD": 3.14, "XABCD": 1.618 }
            
            rule_based_conf = harmonic_ratio_similarity(actuals, ideals, self.CRAB_RANGES)
