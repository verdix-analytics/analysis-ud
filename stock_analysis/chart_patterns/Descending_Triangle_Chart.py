...
from stock_analysis.confidence_models.chart_patterns.descending_triangle_model import (
    load_model,
    learned_confidence,
    descending_triangle_score,
)
...
class DescendingTriangleChartPattern(BasePattern):
    enabled = True
    ...
    def _run(self, df: pd.DataFrame) -> dict:
        ...
        low_abs_pct_slope = abs(low_pct_slope)

        low_std = float(np.std(low_y))
        low_mean = float(np.mean(low_y))
        low_deviation = safe_ratio(low_std, low_mean)
        ...
        confirmed = (
            confidence >= 50
            and float(df.iloc[-1]["Close"]) < float(support_now)
        )
