...
from stock_analysis.confidence_models.chart_patterns.diamond_top_model import (
    load_model,
    learned_confidence,
    diamond_top_score,
)
...
class DiamondTopChartPattern(BasePattern):
    enabled = True
    ...
    def _run(self, df: pd.DataFrame) -> dict:
        ...
        center_dist = abs(max_idx - center_target)
        ...
        right_top = max(right_highs) if len(right_highs) > 0 else max(prices)

        right_width = safe_ratio(
            right_top - right_boundary,
            max(abs(right_top), 1e-9),
        )
