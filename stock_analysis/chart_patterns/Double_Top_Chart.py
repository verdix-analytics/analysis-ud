...
from stock_analysis.confidence_models.chart_patterns.double_top_model import (
    load_model,
    learned_confidence,
    double_top_score,
)
...
class DoubleTopChartPattern(BasePattern):
    enabled = True
    ...
    def _run(self, df: pd.DataFrame) -> dict:
        ...
        avg_high = (h1 + h2) / 2
        peak_diff_pct = safe_ratio(abs(h1 - h2), avg_high)
        drop_ratio = safe_ratio(avg_high - l1, avg_high)
        ...
        confirmed = (
            confidence >= 50
            and float(df.iloc[-1]["Close"]) < float(l1)
        )
