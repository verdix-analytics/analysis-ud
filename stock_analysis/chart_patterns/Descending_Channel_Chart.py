...
from stock_analysis.confidence_models.chart_patterns.descending_channel_model import (
    load_model,
    learned_confidence,
    descending_channel_score,
)
...
class DescendingChannelChartPattern(BasePattern):
    enabled = True
    ...
    def _run(self, df: pd.DataFrame) -> dict:
        ...
        upper_neg_pct = max(-upper_pct, 0.0)
        lower_neg_pct = max(-lower_pct, 0.0)
        ...
        width_stability_score = max(
            0.0,
            1.0 - min(width_change_ratio / 0.5, 1.0),
        )

        touch_richness_score = min((touch_high + touch_low) / 8.0, 1.0)

        rule_score = descending_channel_score(
            upper_neg_pct_slope=upper_neg_pct,
            lower_neg_pct_slope=lower_neg_pct,
