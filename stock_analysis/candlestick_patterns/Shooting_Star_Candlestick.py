...
class ShootingStarCandlestickPattern(BasePattern):
    enabled = True
    ...
    def _run(self, df: pd.DataFrame) -> dict:
        ...
        # --- 1. Long upper wick (max 50) ---
        scores["upper_wick"] = max(0, min(50, (upper_body_ratio - 2.2) / (5.0 - 2.2) * 50))

        # --- 2. Small lower wick (max 25) ---
        scores["lower_wick"] = max(0, min(20, (0.4 - lower_body_ratio) / 0.4 * 20))

        # --- 3. Small body relative to full range (max 25) ---
        scores["body_smallness"] = max(0, min(15, (0.35 - body_ratio) / 0.35 * 15))


        # --- 4. Body should sit near the lower end of the candle (max 10) ---
        scores["body_position"] = max(0, min(15, (0.4 - body_top_pos) / 0.4 * 15))
