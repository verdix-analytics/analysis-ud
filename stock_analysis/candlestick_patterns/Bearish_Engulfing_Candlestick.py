...
class BearishEngulfingCandlestickPattern(BasePattern):
    enabled = True
    ...
    def _run(self, df: pd.DataFrame) -> dict:
        ...
        # Strict bearish engulfing body condition
        shape_ok = (
        ...
        # 1. Body engulf strength (max 70)
        scores = {}
        scores["body_engulf"] = min(30, (body_ratio-1.15) / (2.5 - 1.15) * 30)
        scores["body_quality"] = min(20, curr_body_ratio * 20)
        
        curr_upper_shadow = curr["High"] - curr["Open"]
        upper_shadow_ratio = safe_ratio(curr_upper_shadow, curr_range)
        scores["upper_shadow"] = max(0, (0.15 - upper_shadow_ratio) / 0.15 * 15)
        
        lookback = df.iloc[:-2]
        trend_move = safe_ratio(
            prev["Close"] - lookback.iloc[0]["Open"],
